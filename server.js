...
async function procesarInformacion(data, condicion, filePath) {
  try {
    ...
    const responseFilePath = path.join(
      FilePathRespuesta,
      `${path.basename(`${filePath}.txt`, ".txt")}-RESP.json`
    );
    ...
    const responseFilePathCufe = path.join(
      FilePathRespuesta,
      `${path.basename(`${filePath}`, ".txt")}-CUFE.txt`
    );
    await fs.promises.writeFile(
      responseFilePathCufe,
      JSON.stringify(response.data.cufe ?? response.data.cude, null, 2).replace(
        /^"|"$/g,
        ""
      ),
      "utf8"
    ...
    if (message == "The given data was invalid.") {
      ...
      notificación(
        StatusDescription,
        model,
        `http://localhost:5173/${JSON.stringify(model)}`
      );
    } else {
      var model = {
        message,
    ...
    for (const fileUrl of filesToDownload) {
      if (fileUrl) {
        ...
        const fileData = await axios.get(
          `http://aristafe.com:81/api/download/${data.establishment_nit}/${fileUrl}`,
          { responseType: "stream" }
        );
